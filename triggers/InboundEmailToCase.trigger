trigger InboundEmailToCase on EmailMessage (before insert) {  
    ...
    for(EmailMessage em: Trigger.New){ 
      ...
      if((emailids.contains('internationalparts@servtek.com') || emailids.contains('generalparts@milacron.com') || emailids.contains('repairparts_na@milacron.com')) && em.Subject != null && isGroupAvailable)
      {
          ...
          if(!string.isBlank(subjectActual))
          {
                ...
                try{
                    ...
                    existingCases = [Select Id, CaseNumber, Subject, Owner_Profile_Name__c, CreatedDate, SuppliedEmail, Status
                                     FROM Case 
                                     WHERE RecordTypeId = :RecordTypeId AND Subject LIKE :subjectActual AND First_Queue__c IN :mapGrp.keyset() LIMIT 1000];                    
                    ...
                    if(existingCases != null && existingCases.size() > 1){
                        for(Case ec: existingCases){
                            if(ec != null){
                                ...
                                if(em.TextBody != null){    
                                    if((em.TextBody.Contains('internationalparts@servtek.com') || em.TextBody.Contains('generalparts@milacron.com') || em.TextBody.Contains('repairparts_na@milacron.com')) && isExistingCase){
                                        isExistingCase = true;
                                    }
