trigger CheckEmailToCase2 on Case (after insert) {
           ...
           if(grpl!=null && grpl.size()>0){
            ...
            if(c.Subject != null){  
                if(c.Subject.containsIgnoreCase('RE:') || c.Subject.containsIgnoreCase('FWD:') || c.Subject.containsIgnoreCase('FW:')){                                                                                                        
                    ...
                    matchingCases = [Select Id, CaseNumber, Subject, RecordTypeId, CreatedDate, OwnerId, Status, SuppliedEmail, Description from Case where Subject LIKE :subjectActual  AND RecordTypeId = :CSRecordId AND Id != :Trigger.New];
                    System.debug(matchingCases);
                    if(matchingCases != null && matchingCases.size() > 0){   
                        for(Case mc: matchingCases){
                            if(mc.RecordTypeId == CSRecordId &&  mc.Status != 'Closed - Issue Resolved' && mc.CreatedDate >= c.CreatedDate.AddDays(-7) && mc.SuppliedEmail == c.SuppliedEmail && c.Description.Contains(mc.Description)){
